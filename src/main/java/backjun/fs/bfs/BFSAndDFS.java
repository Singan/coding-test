...
public class BFSAndDFS {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(dfs(node,start));
        //BFS
        visited = new boolean[n + 1];
        System.out.println(bfs(node,start));
    }

    public static String bfs(int[][] node, int start) {
        ...
        while (!queue.isEmpty()) {
            int poll = queue.poll();
            result += poll + " ";
            for (int i = 0; i < node[poll].length; i++) {
    ...
    public static String dfs(int[][] node, int start) {
        visited[start] = true;
        String result = start+" ";

...
public class ATM {
    ...
    // 3번 사람은 1+2+3+3+4 = 13분이 걸리게 된다. 각 사람이 돈을 인출하는데 필요한 시간의 합은 1+3+6+9+13 = 32분이다. 이 방법보다 더 필요한 시간의 합을
    // 최소로 만들 수는 없다.
    //
    //줄을 서 있는 사람의 수 N과 각 사람이 돈을 인출하는데 걸리는 시간 Pi가 주어졌을 때, 각 사람이 돈을 인출하는데 필요한 시간의 합의 최솟값을 구하는
    // 프로그램을 작성하시오.
    //
    //입력
    ...
    public static void main(String[] args) throws Exception {
        ...
        int result = 0;
        int[] numbers = Arrays.stream(br.readLine().split(" ")).mapToInt(value -> Integer.parseInt(value)).toArray();
        Arrays.sort(numbers);
